...
The [JSON report](../public/receipt-demo-v1.json) is a timestamped RPC verification record, not a portable certificate. Devnet can reset, and a later lookup can become unavailable. The payment does not prove the wallets' real-world owners or service delivery. An approved 92.76-second silent product-tour video has been recorded, but it does not show wallet approval; no hosted video URL or confirmed Hackalaunch submission is available. Track these in [SUBMISSION.md](SUBMISSION.md) and the dated [verification record](VERIFICATION.md).
